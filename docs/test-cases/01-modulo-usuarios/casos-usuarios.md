...
> **Nota sobre evidencias:** Las capturas de pantalla referenciadas son archivos locales ubicados en `./evidencias/`. Para consultarlas, clonar el repositorio y navegar a la carpeta correspondiente. No se encuentran disponibles como URLs públicas en esta versión.
...
**Resultado obtenido**
**El sistema creó el registro con el valor `ABCDE!!!` en la columna `numero_documento` sin mostrar ningún error.** No existe validación de formato en el `UsuarioSerializer` ni en el formulario Angular.
...
|-----|-----------|-------------|-----------------|
| 1 | Crítico | Agregar un `RegexValidator` en el campo `numero_documento` del `UsuarioSerializer` de DRF para aceptar solo dígitos (`^\d+$`). Complementar con validación equivalente en el formulario Angular. | Backend / Frontend |
| 2 | Importante | Agregar restricción de longitud mínima y máxima al campo `numero_documento` según el tipo de documento (ej. CC: 7-10 dígitos). | Backend |
| 3 | Mejora | Fortalecer la política de contraseñas: considerar mínimo 10 caracteres y verificación contra listas de contraseñas comunes con `CommonPasswordValidator` de Django. | Backend |
| 4 | Mejora | Registrar en auditoría la creación de usuarios, incluyendo el administrador que realizó la acción, la IP de origen y la fecha/hora. | Backend |
| 5 | Buenas prácticas | Implementar confirmación de contraseña en el formulario Angular para evitar errores tipográficos al momento de crear el usuario. | Frontend |
